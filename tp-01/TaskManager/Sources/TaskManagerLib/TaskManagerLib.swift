...
public func createCorrectTaskManager() -> PTNet {
    ...
    let processPool = PTPlace(named: "processPool")
    let inProgress  = PTPlace(named: "inProgress")
    let checkTask = PTPlace(named: "checkTask") // New place created

    // Transitions
    let create      = PTTransition(
        named          : "create",
        preconditions  : [],
        postconditions : [PTArc(place: taskPool), PTArc(place: checkTask)]) //we also add a tocken to checkTask
    let spawn       = PTTransition(
        named          : "spawn",
        preconditions  : [],
        postconditions : [PTArc(place: processPool)])
    ...
    let exec       = PTTransition(
        named          : "exec",
        preconditions  : [PTArc(place: taskPool), PTArc(place: processPool), PTArc(place: checkTask)],
        postconditions : [PTArc(place: taskPool), PTArc(place: inProgress)]) // Doesen't stay when the task is in progress
    let fail        = PTTransition(
        named          : "fail",
        preconditions  : [PTArc(place: inProgress)],
      postconditions : [PTArc(place: checkTask)]) // in case of failure a token is recreated in checkTask

    // P/T-net
    return PTNet(
        places: [taskPool, processPool, inProgress, checkTask],
        transitions: [create, spawn, success, exec, fail])
}
